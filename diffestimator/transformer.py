...
class LoFTRBlockLayer(nn.Module):
    def __init__(self,
                 ...
                 nhead,
                 attention='linear'):
        super(LoFTRBlockLayer, self).__init__()
...
class LOFTREncoder(nn.Module):
    """A Local Feature Transformer (LoFTR) module."""

    def __init__(self, num_layer):
        super(LOFTREncoder, self).__init__()

        self.config = Encoder_Config()
        self.d_model = self.config.dim_model
        self.num_head = self.config.num_head
        self.attention = self.config.attention
        encoder_layer = LoFTRBlockLayer(self.d_model, self.num_head, self.attention)
...
class LOFTRDecoder(nn.Module):
    """A Local Feature Transformer (LoFTR) module."""

    def __init__(self, num_layer):
        super(LOFTRDecoder, self).__init__()

        self.config = Encoder_Config()
        self.d_model = self.config.dim_model
        self.num_head = self.config.num_head
        self.attention = self.config.attention
        decoder_layer = LoFTRBlockLayer(self.d_model, self.num_head, self.attention)
        self.layers = nn.ModuleList([copy.deepcopy(decoder_layer) for _ in range(num_layer)])
        self._reset_parameters()
