...
class FeatPointCloud:
    ...
    def update_ply(self, kps: torch.Tensor, kp_feat: torch.Tensor, neigbor_pts: list, neigbor_feats: list):
        """
        ...
        for kp in kps:
            ...
            # Set the neigbor points
            neigbor_idx = 0 
            for neigbor_pt in neigbor_pts[kp_idx]:
                distance = torch.norm(self._xyz - neigbor_pt, dim=1)
                closest_index = torch.argmin(distance).item()
                self._semantic_feature[closest_index] = neigbor_feats[kp_idx][neigbor_idx].unsqueeze(-1)
                neigbor_idx = neigbor_idx + 1
