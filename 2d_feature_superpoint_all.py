...
###############################
# python 2d_feature_superpoint_all.py -s datasets/wholehead/ -m output_wholescene/img_2000_head --iteration 15000  
################################
...
def getTopKindices(score_map: Tensor, top_k=4800):
    """
...
def superpoint_match(feats1, feats2, min_cossim = 0.82):
...
def localize_set(model_path, name, scene, gaussians, pipeline, background, args):
    ...
    for _, view in enumerate(tqdm(views_test, desc="Rendering progress")):
        ...
        feat  = feature_extractor(query_img[None].cuda())
        orig_feat_map = feat["original_feature_map"][0]
        scores = feat["scores"][0]
        
         # Extract sparse features
        kp_query = getTopKindices(scores.squeeze(0), 4800)
        desc_query = getTopKFeat(kp_query, orig_feat_map)
        ...
        idxs0, idxs1 = superpoint_match(torch.tensor(desc_query).to("cpu"), proj_p_feature.to("cpu"), min_cossim=0.82 )
