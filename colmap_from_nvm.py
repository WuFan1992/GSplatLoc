...
"""
For Cambridge dataset used only 

Command : python colmap_from_nvm --nvm_path datasets/cambridge/KingsCollege/reconstruction.nvm --colmap_path datasets/cambridge/KingsCollege/sparse/0
"""
...
def read_nvm_model(nvm_path, width=1920, height=1080, skip_points=False):
    ...
    for idx, data in enumerate(image_data):
        ...
        #name, f, qx, qy, qz, qw, cx, cy, cz, k, _ = data
        ...
        if idx in image_idx_to_keypoints:
            # NVM only stores triangulated 2D keypoints: add dummy ones
            keypoints = image_idx_to_keypoints[idx]
            point2D_idxs = np.array([d[0] for d in keypoints])
            tri_xys = np.array([[x+960, y+540] for _, x, y, _ in keypoints])
